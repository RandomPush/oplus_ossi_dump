...
swap_size_mb=0
nandswap_enable=`getprop persist.sys.oplus.nandswap`
data_type=erofs
...
function configure_zram_parameters()
{
	...
	if [ -f /sys/block/zram0/disksize ]; then
		...
		if [[ $hybridswap_has_insmod -eq 0 ]]; then
			zram_increase=0
		fi
...
function nandswap_init()
{
	...
	# use fake minimal size to init hybridswap
	if [ $nandswap_enable == "false" ]; then
