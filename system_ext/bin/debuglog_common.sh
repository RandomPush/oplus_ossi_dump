...
function initLogSizeAndNums() {
    ...
    if [ ${FreeSize} -ge ${GSIZE} ]; then
        ...
        if [ "${tmpKernel}" != "" ]; then
            ...
            tmpKernelSize=`set -f;array=(${tmpKernel//|/ });echo "${array[0]}"`
            tmpKernelCount=`set -f;array=(${tmpKernel//|/ });echo "${array[1]}"`
            KERNEL_SIZE=`echo ${tmpKernelSize} | awk '{printf("%d",$1*1024)}'`
            KERNEL_MAXMUM=`echo ${FreeSize} 1 50 ${KERNEL_SIZE} | awk '{printf("%d",$1*$2/$3/$4)}'`
            echo "tmpKernelSize=${tmpKernelSize}; tmpKernelCount=${tmpKernelCount} KERNEL_SIZE=${KERNEL_SIZE} KERNEL_MAXMUM=${KERNEL_MAXMUM}"
            if [ ${KERNEL_MAXMUM} -ge ${tmpKernelCount} ]; then
                KERNEL_MAXMUM=${tmpKernelCount}
            fi
            echo "last KERNEL_MAXMUM=${KERNEL_MAXMUM}"
        fi
        ...
        if [ "${tmpTcpdump}" != "" ]; then
            ...
            echo "tmpTcpdumpSize=${tmpTcpdumpCount}; tmpTcpdumpCount=${tmpTcpdumpCount} tcpdumpSize=${tcpdumpSize} tcpdumpCount=${tcpdumpCount}"
