...
function checkNumberSizeAndMove(){
    ...
    LOG_LIMIT_NUM="$3"
    LOG_LIMIT_SIZE="$4"
    traceTransferState "CNSAM:FROM ${LOG_SOURCE_PATH}"
    LIMIT_NUM=500
    #500*1024KB
    LIMIT_SIZE="512000"
...
function transfer_logtype_call() {
    moduleConfig=(common performance bluetooth power thirdpart stability recovery audio touch)
    transferModules
}
...
function collect_function_common_debugging() {
    ...
    if [ -d ${DATA_DEBUGGING_PATH} ]; then
        if [[ "${ALWAYSON_ENABLE}" = "true" ]]; then
            ...
            ALL_SUB_DIR=`ls ${DATA_DEBUGGING_PATH} | grep -v SI_stop | grep -v minilog`
    ...
    if [ -d ${MTK_DEBUGLOGGER_PATH} ]; then
        if [[ "${ALWAYSON_ENABLE}" = "true" ]]; then
            ...
            mv -f ${MTK_DEBUGLOGGER_PATH} ${newpath}/${TRANSFER_MODULE}
            traceTransferState "TRANSFERDEBUGLOGGERLOG:mv ${MTK_DEBUGLOGGER_PATH} done"
        fi
    fi
...
function collect_function_stability_hprofDump() {
    ...
    TARGET_DATA_HPROF_LOG=${newpath}/${TRANSFER_MODULE}/assistlog
...
function collect_function_touch_default() {
    checkNumberSizeAndMove "/sdcard/tp_debug_info.txt" "${newpath}/tp_debug_info.txt"
}
...
function collect_function_common_systemApp() {
    ...
    checkSmallSizeAndCopy "${MIDAS_LOG}" "${TRANSFER_MODULE}/os/com.oplus.onetrace"
