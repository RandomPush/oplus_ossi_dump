...
AGG_BATTERY_AGENT_DAILY_TAB as (
	...
	from (
		select case
			...
			end as battery_fcc_check
			, case
				when soh <> -1 or oplus_soh <> -1 then 'pass'
				else 'fail'
			end as soh_check,battery_health
			, case
				when battery_health <> '' then 'pass'
				else 'fail'
			end as battery_health_check,cycle_count
			, case
				when cycle_count >= 0 or native_cycle_count >= 0 or oplus_cycle_count >= 0 then 'pass'
				else 'fail'
			end as cycle_count_check
	...
	from (
		...
		from (
			select count(*) as fail_cnt
				...
				', battery_fcc :' || battery_fcc || battery_fcc_check || ', soh :' || soh_check || ', battery_health :' || battery_health || battery_health_check ||
				', cycle_count :' || cycle_count_check) as detail
...
AGG_DISPLAY_AGENT_DAILY_TAB as (
	...
	from (
		select case
			...
			end as energy_check,by_sumHist
			, case
	...
	from (
		...
		from (
			select count(*) as fail_cnt
				...
				', by_sumHist :' || by_sumHist_check || ', by_bright :' || by_bright || by_bright_check) as detail
